...
export default class UserService {
  ...
  async getUserInfoService(userId: string): Promise<any> {
    console.log('services->v1->user.service->getUserInfoService');
    ...
    console.log('return form the Dao', user);
  ...
  async getAllUsersService(
    ...
    if (limit <= 0 || offset < 0)
      throw new Error(errorResponse.INVALID_PAGINATION);

    return this.userDao.getAllUsersDao(limit, offset);
  }

  async updateUserService(id: string, updateData: any): Promise<any> {
